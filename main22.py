...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, input_len):
        ...
        input_len = [int(x) for x in input_len.numpy()] if isinstance(input_len, torch.Tensor) else [int(x) for x in input_len]
...
if __name__ == '__main__':
    ...
    # encoder
    enc = Encoder(10, 5)
    inputs = torch.Tensor([[1,2,3,2]]).long()
    inputs_len = torch.Tensor([4]).long()
    print(enc(inputs, inputs_len))
